  ...
  <ul class="lsl-dir-tree" >
    <li v-for="(item, i) in data" :key="i">
      ...
      <a href="javascript:;" @click="item.isFolder?clickTreeNode($event.currentTarget, item.fileList !== undefined?index + ',' + i:'null-dir', true):''">
        <i v-if="item.isFolder" class="layui-icon layui-icon-template" style="font-size: 1rem;margin-left: -0.2rem"></i>
        <span v-if="item.isFolder == false" :style="'color:' + color[item.type][0]">{{color[item.type][1]}}</span>
        <cite :class="item.isFolder ? 'tree-dir-cite' : 'tree-file-cite'">{{item.fileName}}</cite>
      </a>
      <ModelTree @listenClickOneDir="listenClickOneDir" v-if="item.fileList !== undefined" :data="item.fileList" :index="index + ',' + i"></ModelTree>
    </li>
  </ul>
</template>
